...
export default function GameHeader({
  ...
  return (
    <div
      ...
      className="bg-white/10 backdrop-blur-lg border border-white/20 shadow-lg rounded-2xl my-2 mb-3 md:mb-6"
    >
      <div className="max-w-6xl mx-auto px-3 md:px-4 py-2 md:py-3">
        ...
        <div className="hidden md:flex items-stretch justify-between gap-4">
          {/* Left: Question */}
          <div className="flex-1 min-w-0 bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border border-yellow-400/30 rounded-xl px-4 py-3 flex flex-col justify-center">
            <h2 className="text-xl font-bold text-yellow-100 truncate">
              โจทย์ : {questionText}
            </h2>
          ...
          <div className="flex items-center gap-2 px-4 py-3 bg-white/5 rounded-xl border border-white/10">
            {Array.from({ length: 3 }).map((_, i) => (
              <span key={i} className={`text-2xl transition-opacity duration-300 ${i < currentHearts ? "opacity-100" : "opacity-30"}`}>
                ❤️
              </span>
            ))}
          </div>
